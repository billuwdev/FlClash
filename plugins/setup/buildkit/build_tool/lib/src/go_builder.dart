...
String _resolveCc(Target target) {
  ...
  final ccPath = p.join(entries.first.path, 'bin', target.ndkCcName);
  if (Platform.isWindows && File('$ccPath.cmd').existsSync()) {
    return '$ccPath.cmd';
  }
  return ccPath;
}
...
class GoBuilder {
  ...
  String _goLdflags(Target target) {
    final needs16KbAlignment =
        target.goos == 'android' &&
        (target.goarch == 'arm64' || target.goarch == 'amd64');
    if (!needs16KbAlignment) return config.goLdflags;
